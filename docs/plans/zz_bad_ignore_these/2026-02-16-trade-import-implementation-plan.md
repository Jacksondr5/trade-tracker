...
### WS-J: Connector Productionization (Replace Mock Connector Payloads)

Goal: move connector adapters from mock-execution auth payloads to real IBKR/Kraken API retrieval with hardened auth/session handling.

Target files:
- `convex/lib/imports/connectors/ibkr.ts`
- `convex/lib/imports/connectors/kraken.ts`
- `convex/lib/imports/connectors/http_client.ts`
- `convex/lib/imports/connectors/rate_limit.ts`
- `convex/importsActions.ts`
- `src/env.ts`

Deliverables:
1. IBKR real fetch path:
- preflight auth/session check
- tickle keepalive around polling
- `/iserver/account/trades` polling with bounded lookback
- explicit `needs_reauth` transition on session/auth failure
2. Kraken real fetch path:
- signed private REST requests to `TradesHistory`
- pagination loop with watermark stop condition
- retry on transient and tier/rate-limit responses
3. Security and config:
- define required env vars in `src/env.ts` (only operational app-level settings)
- continue storing per-user broker credentials/tokens in `brokerageConnections.auth`
4. Observability:
- capture provider response diagnostics in `importJobs.errorSummary`
- surface `degraded` / `maintenance_window` behavior consistently
5. Contract compatibility:
- keep normalized execution contract unchanged so ingestion/UI layers do not change.

Acceptance criteria:
- Manual and scheduled sync use live provider retrieval (no `mockExecutions` dependency).
- IBKR pacing guardrails prevent request bursts and emit clear blocked/degraded states.
- Kraken multi-page imports are idempotent and cursor-safe.
- Failure modes are actionable in Import Errors UI.

Dependencies: WS-C, WS-D, WS-E, WS-F.

---

### WS-K: Credential Management UI (Broker Connection Setup)

Goal: provide a secure user-facing UI for creating, editing, validating, and revoking IBKR/Kraken connection credentials.

Target files:
- `src/app/imports/connections/page.tsx` (new)
- `src/components/imports/connection-form.tsx` (new)
- `src/components/imports/connection-list.tsx` (new)
- `src/components/imports/connection-status-badge.tsx` (new)
- `src/components/imports/import-sync-controls.tsx`
- `src/components/Header.tsx` (optional label/link updates)
- `convex/imports.ts` (extend mutations for validation/revoke flows)

Deliverables:
1. Connections page/section under Imports:
- list existing connections (provider, account ref, status, last validated/synced)
- create new IBKR/Kraken connection
- update existing connection auth payload
- disconnect/revoke connection
2. Provider-specific credential forms:
- IBKR fields (session/cookie/token details required by connector)
- Kraken fields (`apiKey`, `apiSecret`, optional `otp`, optional base URL override)
- strict client-side validation for required fields by provider
3. Secure handling UX:
- never display full secrets after save (mask all sensitive values)
- explicit “Replace credentials” action instead of auto-prefilling secrets
- clear warning text about read-only broker permissions
4. Validation workflow:
- “Validate connection” action that runs connector preflight and updates status
- surface `needs_reauth` / `error` / `active` state transitions in UI
5. Imports page integration:
- link to connection management from `/imports`
- show empty-state CTA to add first connection before sync usage

Acceptance criteria:
- User can self-serve add/edit/remove IBKR and Kraken credentials from UI.
- Secrets are not rendered back in plaintext after save.
- Validation action updates connection status with actionable messaging.
- Sync controls only allow active/validated connections.

Dependencies: WS-E, WS-F, WS-J.
...
7. WS-J (required before first release sign-off)
8. WS-K (required before first release sign-off)
...
Gate 5 (Release 1 connector production readiness):
- WS-J merged.
- Live IBKR/Kraken retrieval paths validated in non-mock environment.
- Import errors reflect real provider auth/session/rate-limit failures clearly.

Gate 6 (Release 1 credential UX readiness):
- WS-K merged.
- Users can configure/validate connections without direct DB edits.
- No plaintext secret leakage in UI renders/logs.
...
6. Mock connector path accidentally ships to production.
- Mitigation: WS-J marked release-blocking gate; remove or feature-flag mock execution path before release.

7. Credential leakage risk in frontend logs/UI.
- Mitigation: WS-K requires masked secret rendering, explicit replacement flows, and no raw secret echo after save.
