...
import { formatCurrency, formatDate, isTradeReadyForAcceptance } from "../utils";
...
interface InboxTableProps {
  ...
  onInlineTradePlanChange: (inboxTradeId: Id<"inboxTrades">, value: string) => void;
  ...
  if (inboxTrades.length === 0) {
    return (
      <div className="rounded-lg border border-slate-700 bg-slate-800 p-8 text-center">
        <p className="text-slate-11">No trades pending review. Upload a CSV to get started.</p>
      </div>
    );
  }
      ...
      <table className="w-full table-auto">
        <thead className="bg-slate-800">
          <tr>
            ...
            <th className="text-slate-11 px-4 py-3 text-right text-sm font-medium">Qty</th>
            <th className="text-slate-11 px-4 py-3 text-right text-sm font-medium">Value</th>
            ...
            <th className="text-slate-11 px-4 py-3 text-left text-sm font-medium">Notes</th>
          ...
          {inboxTrades.map((trade) => (
            ...
            >
              ...
              <td className="text-slate-12 whitespace-nowrap px-4 py-3 text-sm font-medium">
                {trade.ticker ?? "---"}
                {(trade.validationErrors.length > 0 || trade.validationWarnings.length > 0) && (
                  <div className="mt-1 space-y-1 text-xs">
                    {trade.validationErrors.slice(0, 2).map((error, index) => (
                      <div key={`${trade._id}-error-${index}`} className="text-red-300">
                        Error: {error}
                      </div>
                    ))}
                    {trade.validationWarnings.slice(0, 2).map((warning, index) => (
                      <div key={`${trade._id}-warning-${index}`} className="text-amber-300">
                        Warning: {warning}
                      </div>
                    ))}
                  </div>
              ...
              <td className="px-4 py-3 text-sm">
                <select
                  ...
                  onChange={(e) => onInlineTradePlanChange(trade._id, e.target.value)}
              ...
              <td className="px-4 py-3 text-sm">
                <input
                  ...
                  onChange={(e) => onInlineNotesChange(trade._id, e.target.value)}
              ...
              <td className="whitespace-nowrap px-4 py-3 text-right text-sm">
                <div className="flex justify-end gap-1">
                  <button
                    ...
                    title={isTradeReadyForAcceptance(trade) ? "Accept" : "Missing required fields"}
                  >
