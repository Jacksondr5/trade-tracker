...
export default function TradePlanDetailPageClient({
  ...
  const updateNote = useMutation(api.tradePlanNotes.updateNote);
  ...
  const handleSaveSymbol = async () => {
    ...
    const trimmedSymbol = instrumentSymbol.trim();

    try {
      await updateTradePlan({
        tradePlanId,
        instrumentSymbol: trimmedSymbol,
      });
      setInstrumentSymbol(trimmedSymbol.toUpperCase());
      ...
      setInstrumentSymbolError(
        error instanceof ConvexError
          ? typeof error.data === "string"
            ? error.data
            : "Failed to save instrument symbol"
          : error instanceof Error
            ? error.message
            : "Failed to save instrument symbol",
      );
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-6 rounded-lg border border-slate-700 bg-slate-800 p-4">
        ...
        {/* Instrument Symbol */}
        <div className="mt-3">
          <label htmlFor="instrument-symbol" className="mb-1 block text-xs uppercase tracking-wide text-slate-11">
            Instrument Symbol
          </label>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <input
              ...
              maxLength={20}
              className="w-full max-w-xs rounded border border-slate-600 bg-slate-700 px-3 py-2 text-slate-12"
      ...
      <NotesSection
        ...
        onUpdateNote={async (noteId, content) => {
          await updateNote({ noteId: noteId as Id<"tradePlanNotes">, content });
        }}
      />
